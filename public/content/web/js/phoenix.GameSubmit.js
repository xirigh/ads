        ...
        getSubmitData: function () {
            ...
            for (; i < len; i++) {
                result['balls'].push({
                    ...
                    'graduationCount': ballsData[i]['graduationCount'],
                    'selectMedia': ballsData[i]['selectMedia']
                });
            }
            result['orders'] = [];
            //非追号
        ...
        afterSubmitSuccess: function () {
            ...
            try {   
                ...
                $.ajax({
                    ...
                    success: function (re) {
						if($.isEmptyObject(re)){
							...
							$.each(re, function (i) {
							    htmls += "<tr><td><a target=\"_black\" href=\"/gameUserCenter/queryOrderDetail?orderId=" + re[i].BettingID + "\"> " + re[i].IssueNo + "</a></td>";
							    htmls += "<td><span class='price'><dfn>&yen;</dfn>" + me.formatMoney(re[i].BettingMoney) + "</span></td>";

							    if (Number(re[i].State) == 4) {
							        htmls += "<td >中<span class='price color-red'><dfn>&yen;</dfn>" + me.formatMoney(re[i].Reward ) + "</td></span></tr>";
								} else {
							        htmls += "<td >" + re[i].StateName + "</td></tr>";
								}
                ...
                $.ajax({
                    ...
                    success: function (re) {
						if($.isEmptyObject(re)){
							...
							$.each(re, function (i) {
							    htmlb += "<tr><td><a target=\"_black\" href=\"/gameUserCenter/queryPlanDetail?orderId=" + re[i].BettingID + "&AddTime=" + re[i].AddTime2FileTime + "\"> " + re[i].IssueNo + "</a></td>";
							    htmlb += "<td>" + Number(re[i].CompleteIssueCount) + "/" + Number(re[i].BuyIssueCount) + "</td>";
							    htmlb += "<td><span class='price'><dfn>&yen;</dfn>" + me.formatMoney(re[i].BettingMoney ) + " </span></td>";
							    htmlb += "<td >" + re[i].StateName + "</td></tr>";
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    cache: false,
                    url: Games.getCurrentGame().getGameConfig().getInstance().getHistoryOpenUrl(),
                    cache: false,
                    success: function (re) {
                        if ($.isEmptyObject(re)) {
                            return;
                        } else {
                            $('#program-history-open > tbody').html('');
                            $.each(re, function (i) {
                                htmlc += "<tr> <td>" + re[i].IssueNo + "</td><td><b class='price'>" + re[i].LotteryOpenNo + "</b></td></tr>";
                             
                            });
                        }

                        $('#program-history-open > tbody').append(htmlc);
                    }
                });
		...
		balanceInquiry : function(htmlObj){
			$.ajax({
				...
				success: function (data) {
					...
					var _userBallF = phoenix.util.formatMoney( _userBall) ;					
					$("#" + htmlObj).html(_userBallF);
					$("#spanBallIntegral").html(data.UserIntegral);
        ...
        submitData: function () {
            ...
            message.show({
                ...
                confirmFun: function () {
					...
					$.each(data.balls, function (inx, ele) {
					    var reg = /[\u4E00-\u9FA5]/;
					    if (ele.ball.length > 100 || !ele.ball.match(reg))
					    {
					        ele.ball = $.base64('encode', zip_deflate(ele.ball));
					        ele.isEncory = true;
					    }
					    else
					    {
