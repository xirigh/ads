...
(function(host, name, Event, undefined){
	var defConfig = {
	    ...
	    //返点设置
		FanDianBtnDom: '#J-change-fanDian',
	    //返点模式
		fanDianModelDom:"#J-balls-statistics-fandianModel",
        //奖金
		fandianJiangJinDom:"#J-balls-statistics-jiangjin",
		//总金额
		...
		moneyUnit:1,
		//元角模式对应的中文
		moneyUnitData:{'0.001':'厘','0.01':'分','0.1':'角','1':'元'},
		//单注价格
		onePrice:2,
		//倍数
		multiple:1,
	...
	var pros = {
		...
		initControl: function(cfg){
			...
			me.onePrice = Games.getCurrentGame().getGameConfig().getInstance().PerBettingPrice();
			...
			me.moneyUnitDom.addEvent('change', function(e, value, text){
			    var gamelimit = 99999;// Games.getCurrentGame().getDynamicConfig()['gamelimit'][me.gameMethodName]['maxmultiple'],
					gameMoneyUnit = Number(value);
				
				me.setMoneyUnit(Number(value));
				$.cookie('MoneyUnit', Number(value), { expires: 30 });
				me.updateData({'lotterys':Games.getCurrentGame().getCurrentGameMethod().getLottery(),'original':Games.getCurrentGame().getCurrentGameMethod().getOriginal()}, Games.getCurrentGame().getCurrentGameMethod().getGameMethodName());
				if(gameMoneyUnit == 1){
		...
		getFandianBtnDom: function () {
		    var me = this, cfg = me.defConfig;
		    return me.FanDianBtnDom || (me.FanDianBtnDom = $(cfg.FanDianBtnDom));
		},
		setEnableFandianBtnDom: function (isEnable) {
		    var me = this, cfg = me.defConfig;
		    if (isEnable)
		    {
		        me.getFandianBtnDom().click(function () {
		            me.setFanDianBtnClickDom();
		        });
		    }
		    else
		    {
		        me.getFandianBtnDom().unbind();
		    }
		...
		updateData:function(data, name){
			...
			{
			    count = count * Games.getCurrentGame().getCurrentGameMethod().getRenXuanCheckBoxPositionNum();
			}
		...
		getResultData:function(){
			...
			var lotteryMap = {
			    ...
			    multiple: me.multiple,
			    graduationCount: me.getGraduationCount(),
			    //单价
			...
			var lotteryCount = 0;
			if (Games.getCurrentGame().getCurrentGameMethod() != null && Games.getCurrentGame().getCurrentGameMethod().defConfig.isRenxuanCheck) {
			    lotteryCount = lotterys['lotterys'].length * Games.getCurrentGame().getCurrentGameMethod().getRenXuanCheckBoxPositionNum();
			    var selectMedia = Games.getCurrentGame().getCurrentGameMethod().getRenXuanChenkSelMedia();
			    var selectMediaText = Games.getCurrentGame().getCurrentGameMethod().getRenXuanChenkSelMediaText();
			    lotteryMap['selectMedia'] = selectMedia;
			    lotteryMap['selectMediaText'] = selectMediaText;
			}
			else
			{
			    lotteryCount = lotterys['lotterys'].length;
			}
			if (lotteryCount < 1) {
			    return {};
			}
			
			lotteryMap['num'] = lotteryCount;

			return lotteryMap;
			
		},
	    //得到用户选择返点
		getGraduationCount: function () {
		   ...
		   //缓存名称必须和手动加载的一致
		    var detailData = Games.cacheData['GameReward'][url];
		    if (detailData.IsRewardFixed) {
		        return $.cookie('AdjustRewardIsFix') ? $.cookie('AdjustRewardIsFix') :0 ;
		    }
		    else {
		        return $.cookie('AdjustReward')?$.cookie('AdjustReward'):0;
		    }
	    ...
	    //奖金
		getJiangJinDom: function () {
		    var me = this, cfg = me.defConfig;
		    return me.fandianJiangJinDom || (me.fandianJiangJinDom = $(cfg.fandianJiangJinDom));
		},
		setJiangJinDom: function (v) {
		    var me = this;
		    me.getJiangJinDom().html(v);
		    ...
		    if (Games.cacheData['gameTips'][url]) {
		        var tipsData = Games.cacheData['gameTips'][url];
		        var tipExamData = tipsData.example.replace('{$奖金}', v);
		        Games.getCurrentGame().getCurrentGameMethod().setEthodExampleDom(tipExamData);
		    }
		    
		},
	    //返点模式
		getFanDianModelDom: function () {
		    var me = this, cfg = me.defConfig;
		    return me.fanDianModelDom || (me.fanDianModelDom = $(cfg.fanDianModelDom));
		},
		setFanDianModelDom: function (v) {
		    var me = this;
		    me.getFanDianModelDom().html(v);
		},
		reSet:function(){
			var me = this,cfg = me.defConfig;
		...
		setFanDianBtnClickDom: function () {
		    var me = this, cfg = me.defConfig;
