...
(function(host, name, Event, undefined){
	...
	var pros = {
				...
				success:function(result){
					if(Number(result['isSuccess']) == 1){
						...
						if(typeof data['gameTips'] != 'undefined'){
							...
							var tipExamData = data.gameTips.example.replace('{$奖金}', Games.getCurrentGameStatistics().getJiangJinDom().html());
							me.setEthodExampleDom(tipExamData);
						}
						//冷热号
	    ...
	    //修改玩实例提示方法
		setEthodExampleDom: function (data) {
		    var me = this,
				cfg = me.defConfig;
		    //玩法实例
		    $(cfg.methodExampleDom).html(data);
		},
		//获取冷热号码、遗漏数据
		//@param name  当前玩法类型
		//@param len   当前选择期数
		...
		randomNum:function(){
			...
			order = {
				...
				'num': lotterys.length,
				'graduationCount': Games.getCurrentGameStatistics().getGraduationCount()
			};
		...
		randomLotterys:function(num){
			...
			for (; i < num; i++) {
			    var orders = me.randomNum();
			    if (!orders['graduationCount'])
			    {
			        orders['graduationCount'] = Games.getCurrentGameStatistics().getGraduationCount();
			    }
			    if (Games.getCurrentGame().getCurrentGameMethod() != null && Games.getCurrentGame().getCurrentGameMethod().defConfig.isRenxuanCheck) {
			        lotteryCount = orders['num'] * Games.getCurrentGame().getCurrentGameMethod().getRenXuanCheckBoxPositionNum();
			        selectMedia = Games.getCurrentGame().getCurrentGameMethod().getRenXuanChenkSelMedia();
			        selectMediaText = Games.getCurrentGame().getCurrentGameMethod().getRenXuanChenkSelMediaText();
			        orders['selectMedia'] = selectMedia;
			        orders['selectMediaText'] = selectMediaText;
			        orders['num'] = lotteryCount;
			        if (lotteryCount < 1)
			        {
			    ...
			    Games.getCurrentGameOrder().add(orders);
			}
		},
		//游戏错误提示
		//主要用于进行单式投注错误提示
		...
		initGameReward: function () {
		    var me = this,
			type = me.getGameMethodName(),
			currentGame = Games.getCurrentGame(),
            message = Games.getCurrentGameMessage();

		    var url = Games.getCurrentGame().getGameConfig().getInstance().getGameRewardUrl() + '&gameType=' + type;

		    if (!Games.cacheData['GameReward']) {
		        Games.cacheData['GameReward'] = {};
		    }

		    //缓存名称必须和手动加载的一致
		    if (Games.cacheData['GameReward'][url]) {
		        me.setGameReward(Games.cacheData['GameReward'][url]);
		    }
		    //验证缓存
		    //禁止异步请求数据
		    if (Games.cacheData['GameReward'][url]) { return };
		    //获取游戏相关数据
		    $.ajax({
		        type: "post",
		        url: url,
		        ...
		        success: function (result) {
		            if (Number(result['isSuccess']) == 1) {
		                ...
		                if (typeof data != 'undefined' ) {
		                    Games.cacheData['GameReward'][url] = data;
		                    me.setGameReward(data);
		                }
		               
		            } else {
		                message.show({
		                    type: 'normal',
		                    msg: '用户返点信息加载失败，请刷新网页重新加载！',
		                    data: {
		                        tplData: {
		                            msg: '用户返点信息加载失败，请刷新网页重新加载！'
		                        }
		...
		setGameReward: function (RewardData) {
		    var me = this;
		    var CanAdjust = Games.getCurrentGame().getGameConfig().getInstance().CanAdjust();
		    var PerBettingPrice = Games.getCurrentGame().getGameConfig().getInstance().PerBettingPrice();
		    var PerGraduation = Games.getCurrentGame().getGameConfig().getInstance().PerGraduation();
		    var BasicReward = Games.getCurrentGame().getGameConfig().getInstance().BasicReward();
		    var YuanJiaoFen = Games.getCurrentGameStatistics().getMoneyUnitDom().getValue();
		    var RewardArr = new Array();

		    var SetAdjust = $.cookie('AdjustReward') ? $.cookie('AdjustReward') : 0;

		    if (RewardData.IsRewardFixed)
		    {
		        Games.getCurrentGameStatistics().setEnableFandianBtnDom(false);
		        if (CanAdjust)
		        {
		            var curretRewiteMode = me.SelectIsRewardFixedAdjust(RewardData);
		            Games.getCurrentGameStatistics().setJiangJinDom(((RewardData.CardinalMoney * Number(curretRewiteMode) / 1000 + RewardData.Modulus) * (PerBettingPrice / 2) * Number(YuanJiaoFen)).toFixed(3));
		           
		        }
		        else
		        {
		            Games.getCurrentGameStatistics().setJiangJinDom((RewardData.CardinalMoney * (PerBettingPrice / 2) * Number(YuanJiaoFen)).toFixed(3));
		           
		        }
		        return ;
		    }

		    Games.getCurrentGameStatistics().setEnableFandianBtnDom(true);
		    if (RewardData.AlgorithmTypeCode =="0105" || RewardData.AlgorithmTypeCode == "0307" || RewardData.AlgorithmTypeCode =="0108")
		    {
		        if (CanAdjust)
		        {
		            me.setSelectAdjust(RewardData);
		           
		       
		            var AdjustRewardMode = BasicReward + PerGraduation * Number(SetAdjust).toFixed(2);
		            RewardArr[0] = (RewardData.CardinalMoney * AdjustRewardMode / 1000 + RewardData.Modulus) * (PerBettingPrice / 2)* Number(YuanJiaoFen);
		            RewardArr[1] = (RewardData.CardinalMoney1 * AdjustRewardMode / 1000 + RewardData.Modulus1) * (PerBettingPrice / 2)* Number(YuanJiaoFen);
		           
		        }
		        else
		        {
		           RewardArr[0] = RewardData.CardinalMoney * (PerBettingPrice / 2)* Number(YuanJiaoFen), RewardData.CardinalMoney1 * (PerBettingPrice / 2)* Number(YuanJiaoFen) ;
		        }
		    }
		    ...
		    {
		        RewardArr[0] = RewardData.CardinalMoney * (PerBettingPrice / 2)* Number(YuanJiaoFen) ;
		    }

		    if (RewardArr.length == 2) {
		        Games.getCurrentGameStatistics().setJiangJinDom(Number(RewardArr[0]).toFixed(3) + "/" + Number(RewardArr[1]).toFixed(3));
		    }
		    else
		    {
		        Games.getCurrentGameStatistics().setJiangJinDom(Number(RewardArr[0]).toFixed(3));
		      
		    }
		},
		setSelectAdjust: function (RewardData) {
		    ...
		    var BasicReward = Games.getCurrentGame().getGameConfig().getInstance().BasicReward();
		    var YuanJiaoFen = Games.getCurrentGameStatistics().getMoneyUnitDom().getValue();
		    var userBonus = Games.getCurrentGame().getGameConfig().getInstance().getUserBonus();
		    var BonusGruduation =Games.getCurrentGame().getGameConfig().getInstance().BonusGruduation();
		    var AdjustMaxBonus =Games.getCurrentGame().getGameConfig().getInstance().AdjustMaxBonus();
		    var AdjustMinBonus =Games.getCurrentGame().getGameConfig().getInstance().AdjustMinBonus();
		    var JiaoMaxMode =Games.getCurrentGame().getGameConfig().getInstance().JiaoMaxMode();
		    var FenMaxMode =Games.getCurrentGame().getGameConfig().getInstance().FenMaxMode();
		    var RewardUp =Games.getCurrentGame().getGameConfig().getInstance().RewardUp();
		    var RewardDown = Games.getCurrentGame().getGameConfig().getInstance().RewardDown();

		    var curretBonus = Number(userBonus.toFixed(2))
                              - (SetAdjust * Number(BonusGruduation.toFixed(2))).toFixed(2);

		    if (Number(AdjustMaxBonus) < curretBonus
                || Number(AdjustMinBonus) > curretBonus)
		    {
		        SetAdjust = 0;
		        $.cookie('AdjustReward', SetAdjust, { expires: 30 });
		    }


		    var curretRewiteMode = Number(BasicReward)
                                    + Number(PerGraduation) * SetAdjust;


		    var jiaoAdjust = Number((Number(JiaoMaxMode)
                                 - Number(BasicReward)) / Number(PerGraduation));

		    var fenAdjust = Number((Number(FenMaxMode)
                                - Number(BasicReward)) /Number(PerGraduation));


		    if (YuanJiaoFen == 0.1)
		    {
		        if (curretRewiteMode > Number(JiaoMaxMode))
		        {
		            SetAdjust = jiaoAdjust;
		            $.cookie('AdjustReward', SetAdjust, { expires: 30 });
		        }
		    }
		    else if (YuanJiaoFen == 0.01)
		    {
		        if (curretRewiteMode > Number(FenMaxMode))
		        {
		            SetAdjust = fenAdjust;
		            $.cookie('AdjustReward', SetAdjust, { expires: 30 });
		        }
		    ...
		    if (curretRewiteMode > Number(RewardUp))
		    {
		        SetAdjust = 0;
		        $.cookie('AdjustReward', SetAdjust, { expires: 30 });
		    }
		    else if (curretRewiteMode < Number(RewardDown))
		    {
		        SetAdjust = 0;
		        $.cookie('AdjustReward', SetAdjust, { expires: 30 });
		    }
		    ...
		    curretBonus = Number(userBonus.toFixed(2))
                             - (SetAdjust * Number(BonusGruduation.toFixed(2)));

		    curretRewiteMode = Number(BasicReward)
                        + Number(PerGraduation) * SetAdjust;

		    Games.getCurrentGameStatistics().setFanDianModelDom(Number(curretBonus.toFixed(2)) + "/" + Number(curretRewiteMode.toFixed(0)));

		   
		},
		SelectIsRewardFixedAdjust: function (RewardData) {
		    ...
		    var RewardUp =Games.getCurrentGame().getGameConfig().getInstance().RewardUp();
		    var RewardDown = Games.getCurrentGame().getGameConfig().getInstance().RewardDown();
		    var FixedRewardMode = Games.getCurrentGame().getGameConfig().getInstance().FixedRewardMode();

            
		    var canFixeAdjustCount = Number((FixedRewardMode - BasicReward) /PerGraduation);
            
		    var canBonusAdjustCount = Number(userBonus / BonusGruduation);

		    var canMaxAdjuestCount = Number((RewardUp - BasicReward) / PerGraduation);
		    var canMinAdjuestCount = Number((RewardDown - BasicReward) / PerGraduation);

		    var canMaxBonusAdjuestCount = Number((userBonus-AdjustMinBonus) / BonusGruduation);
		    var canMinBonusAdjuestCount = Number((userBonus-AdjustMaxBonus) / BonusGruduation);
		    ...
		    listInt.push(canFixeAdjustCount);
		    listInt.push(canBonusAdjustCount);
		    listInt.push(canMaxAdjuestCount);
		    listInt.push(canMaxBonusAdjuestCount);

		  
		    if (YuanJiaoFen == 0.1)
		    {
		        listInt.push(jiaoAdjust);
		    }
		    else if (YuanJiaoFen == 0.01)
		    {
		        listInt.push(fenAdjust);
		    }

		 

		    var listIntMin =  new Array();
		    listIntMin.push(canMinAdjuestCount);
		    listIntMin.push(canMinBonusAdjuestCount);
		   

		    var currentAdjuest = 0;
		    if (listInt[0] < listIntMin[0])
		    {
		        currentAdjuest = listIntMin[0];
		    }
		    else
		    {
		        currentAdjuest = listInt[0];
		    }

		    var curretBonus = Number(userBonus.toFixed(2))
                 - (currentAdjuest * Number(BonusGruduation.toFixed(2)));

		    var curretRewiteMode = Number(BasicReward)
                        + Number(PerGraduation) * currentAdjuest;

		 
		    $.cookie('AdjustRewardIsFix', currentAdjuest, { expires: 30 });
		    Games.getCurrentGameStatistics().setFanDianModelDom(Number(curretBonus.toFixed(2)) + "/" + Number(curretRewiteMode.toFixed(0)));
		    

		    return curretRewiteMode;
