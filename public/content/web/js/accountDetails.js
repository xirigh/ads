$(document).ready(function () {
   ...
   dateFilterFn = function (e) {
       var Dt = new phoenix.DatePicker({
           input: this, isShowTime: false, setDisabled: function () {
   ...
   time_s = Number(_arrDate[4]);
    ...
    if ($('#fromDateValue').val() != '') {
        $('#J-date-start').val($('#fromDateValue').val());
    }

    if ($('#toDateValue').val() != '') {
        $('#J-date-end').val($('#toDateValue').val());
    }

    if ($("#searchNameValue")) {
        if ($('#searchNameValue').val() != '') {
            $('#searchName').val($('#searchNameValue').val());
        }
    }
