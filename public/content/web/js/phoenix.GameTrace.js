...
(function(host, name, Event, undefined){
	var defConfig = {
		...
		//追号数据表头
		dataRowHeader:'<tr><th style="width:125px;" class="text-center">序号</th><th><input data-action="checkedAll" type="checkbox"  checked="checked"/> 追号期次</th><th>倍数</th><th>金额</th></tr>',
		//追号数据列表模板
		dataRowTemplate:'<tr><td class="text-center"><#=No#></td><td><input data-action="checkedRow" class="trace-row-checked" type="checkbox" checked="checked"> <span class="trace-row-number" issueCode=<#=issueCode#>><#=traceNumber#></span></td><td><input class="trace-row-multiple" value="<#=multiple#>" type="text" style="width:30px;text-align:center;"> 倍</td><td><span class="price"><dfn>&yen;</dfn><span class="trace-row-money"><#=money#></span> 元</span> </td></tr>',
		//高级追号盈利金额追号/盈利率追号表模板
...
//			});
			...
			trs.each(function(i){
				...
				if(i > 0){
					...
					if(checkboxDom.size() > 0 && checkboxDom.get(0).checked){
						...
						yinglilvDom.text(Number(yinglilv).toFixed(4));
		...
		getResultData:function(){
			...
			trs.each(function(i){
				...
				traceNo.html('').html(i);
				if (traceLastNumber != tracenumber.substr(0, 8) && traceLastNumber != "" && (isGameTypes != 'fc3d' && isGameTypes != 'p5' && isGameTypes != 'ssq' && isGameTypes != '60002')) {//增加隔天期标识
		...
		trace_normal:function(){
			...
			for(;i < times;i++){
				...
				if(numberData['number']){
					rowData = {'No':i+1,'traceNumber':currNumberText,'issueCode':numberData['issueCode'],'multiple':multiple,'money':me.formatMoney(orderAmount * multiple)};
		...
		trace_advanced_fanbei_a:function(maxnum){
			var me = this,
				...
				for(;i < times;i++){
					...
					rowData = {'No':traceNo,'traceNumber':currNumberText,'issueCode':numberData['issueCode'],'multiple':multiple,'money':me.formatMoney(orderAmount * multiple)};
		...
		trace_advanced_yingli_a:function(maxnum){
			...
			for(;i < times;i++){
				...
				rowData = {'No':traceNo,'traceNumber': currNumberText,
							...
							'yinglilv':Number(yinglilv*100).toFixed(4)
							};
			...
			var me = this,
				...
				for(;i < times;i++){
					...
					rowData = {'No':traceNo,'traceNumber': currNumberText,
								...
								'userGroupMoney':me.formatMoney(userGroupMoney * multipleBase),
								'winTotalAmout':me.formatMoney(winTotalAmout),
								'yinglilv':Number(winTotalAmout/orderTotalAmount*100).toFixed(4)
					};
