...
; (function (host, name, Event, undefined) {
    var defConfig = {
        ...
        overLimit: ['<div class="bd text-center"><div class="pop-title"><i class="ico-waring"></i><h4 class="pop-text"><#=msg#><br /></h4></div>',
                        ...
                        '</div>'].join(''),
        //返点设置
        fandianChange: ['<div class="bd  text-center">',
                                '<div class="pop-waring">',
                                    '<i class="ico-success"></i>',
                                    '<h4 class="pop-text"><#=msg#><br /></h4>',
                                    '<div><div class="fandianSilderDivParent"><div class="fandianSilderLeftDiv"><a class="fandianSilderLeftBtn" id="fandianSilderLeftBtn"></a></div><div class="fandianSilderDiv"><div style="top:50%;margin-top:-7px;" id="J-balls-messages-fandianSilder"></div></div><div class="fandianSilderRightDiv"><a class="fandianSilderRightBtn" id="fandianSilderRightBtn"></a></div></div>  </div>',
                                     '<h4 class="pop-text">当前奖金模式: <span id="J-balls-messages-fandianSilderText">9.0/1800</span> <br /></h4>',
                                '</div>',
                            '</div>'].join('')
    ...
    var pros = {
        ...
        rebulidcheckLotters: function (parameter) {
           ...
           // debugger;
            result['content'] = me.formatHtml(parameter['tpl'], parameter['data']['tplData']);
            __printcontent = result['content'] + "购买时间：" + (new Date()).toLocaleString();
            me.show($.extend(result, parameter));
        },
        ...
        rebulidsuccess: function (parameter) {
            ...
            result["confirmText"] = "打 单";

            result['confirmFun'] = function (a,b,c,d) {
                var iframe = document.createElement('IFRAME');
                var doc = null;
                iframe.setAttribute('style', 'position:absolute;width:0px;height:0px;left:-500px;top:-500px;');
                document.body.appendChild(iframe);
                doc = iframe.contentWindow.document;
                doc.write('<div>' + __printcontent + '</div>');
                doc.close();
                    ...
                    success: function (r) {
                        ...
                        if (Number(r['isSuccess']) == 1) {
                            ...
                            if (dataTrace > 0) {
                                ...
                                r.data.tplData["url"] = "/gameUserCenter/queryOrders?time=7";
        ...
        //返点调节
        rebulidfandianChange: function (parameter) {
            ...
            result['confirmFun'] = function () {
                ...
                var temp = Number(($("#J-balls-messages-fandianSilder").slider("value") - Number(BasicReward.toFixed(4))) / Number(PerGraduation.toFixed(4))).toFixed(0);
                $.cookie('AdjustReward', temp, { expires: 30 });
                //获取用户奖金返点
                Games.getCurrentGame().getCurrentGameMethod().initGameReward();
                me.hide();
            };

            result['content'] = me.formatHtml(me.getHtmlfandianChange(), parameter['data']['tplData']);
            me.show($.extend(result, parameter));
            me.initfandianChangeConfig();

        },
        //返点调节
        getHtmlfandianChange: function () {
            var cfg = this.defConfig;
            return cfg.fandianChange;
        },
        //返点调节
        initfandianChangeConfig: function () {
            var me = this;
            ...
            var PerGraduation = Games.getCurrentGame().getGameConfig().getInstance().PerGraduation();
            var BasicReward = Games.getCurrentGame().getGameConfig().getInstance().BasicReward();
            var AdjustShowMaxMode = Games.getCurrentGame().getGameConfig().getInstance().AdjustShowMaxMode();
            ...
            var canAdjust = Number(((Number)((userBonus - AdjustMinBonus) / BonusGruduation) * PerGraduation + BasicReward).toFixed(4));
            if (AdjustShowMaxMode) {
                RewardSlider.Maximum = Number(RewardUp.toFixed(4));
            }
            else {
                if (canAdjust <= Number(RewardUp.toFixed(4))) {
                    RewardSlider.Maximum = canAdjust;
                }
                else {
                    RewardSlider.Maximum = Number(RewardUp.toFixed(4));
                }
            ...
            RewardSlider.Minimum = Number(RewardDown.toFixed(4));
            RewardSlider.SmallChange = Number(PerGraduation.toFixed(4));
            RewardSlider.LargeChange = Number(PerGraduation.toFixed(4));
            RewardSlider.MaxChangeValue = Number(((Number)((userBonus - AdjustMinBonus) / BonusGruduation) * PerGraduation + BasicReward).toFixed(4));


            if (YuanJiaoFen == 0.1) {
                if (JiaoMaxMode < Number(RewardSlider.MaxChangeValue.toFixed(4))) {
                    RewardSlider.MaxChangeValue = Number(JiaoMaxMode);
                }
            }
            else if (YuanJiaoFen == 0.01) {
                if (FenMaxMode < Number(RewardSlider.MaxChangeValue.toFixed(4))) {
                    RewardSlider.MaxChangeValue = Number(FenMaxMode);
                }
            }


            RewardSlider.MinChangeValue = Number((BasicReward - ((AdjustMaxBonus - userBonus) * PerGraduation / BonusGruduation)).toFixed(4));
            var adjust = $.cookie('AdjustReward') ? $.cookie('AdjustReward') : 0;
            RewardSlider.Value = Number(BasicReward.toFixed(4)) + Number(PerGraduation.toFixed(4)) * adjust;

            $("#J-balls-messages-fandianSilder").slider({
                value: Number(RewardSlider.Value),
                min: Number(RewardSlider.Minimum),
                max: Number(RewardSlider.Maximum),
                step: Number(RewardSlider.SmallChange),
                slide: function (event, ui) {
                    me.SetFandianBonus(ui.value);
                }
            });

            $("#fandianSilderRightBtn").click(function () {
                var value = $("#J-balls-messages-fandianSilder").slider("value")
                if (value < Number(RewardSlider.Maximum)) {
                    var changeVal = value + Number(RewardSlider.SmallChange);
                    if (changeVal >= Number(RewardSlider.Maximum)) {
                        changeVal = Number(RewardSlider.Maximum);
                    }
                    $("#J-balls-messages-fandianSilder").slider("value", changeVal);
                    me.SetFandianBonus(changeVal);
                }
            });

            $("#fandianSilderLeftBtn").click(function () {
                var value = $("#J-balls-messages-fandianSilder").slider("value")
                if (value > Number(RewardSlider.Minimum)) {
                    var changeVal = value - Number(RewardSlider.SmallChange);
                    if (changeVal <= Number(RewardSlider.Minimum)) {
                        changeVal = Number(RewardSlider.Minimum);
                    }
                    $("#J-balls-messages-fandianSilder").slider("value", changeVal);
                    me.SetFandianBonus(changeVal);
                }
            ...
            me.SetFandianBonus($("#J-balls-messages-fandianSilder").slider("value"));
        },
        SetFandianBonus: function (silderValue) {
            var me = this;
            var userBonus = Games.getCurrentGame().getGameConfig().getInstance().getUserBonus();
            var BonusGruduation = Games.getCurrentGame().getGameConfig().getInstance().BonusGruduation();
            var AdjustMinBonus = Games.getCurrentGame().getGameConfig().getInstance().AdjustMinBonus();
            ...
            var increaseBonus = BonusGruduation * (BasicReward - Number(silderValue)) / PerGraduation;
            var currentBonus = userBonus + increaseBonus;
            currentBonus = currentBonus > AdjustMaxBonus ? AdjustMaxBonus : currentBonus;
            currentBonus = currentBonus < AdjustMinBonus ? AdjustMinBonus : currentBonus;
            $("#J-balls-messages-fandianSilderText").html(Number(currentBonus.toFixed(4)).toFixed(4) + "/" + silderValue);
